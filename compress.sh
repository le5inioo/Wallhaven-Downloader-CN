...
# 7. 单个图片处理（核心：按顺序执行格式转换→分辨率调整→体积压缩）
process_image() {
    local img_path=$1
    ...
    # 第一步：格式转换 - 非JPG格式统一转为JPG
    if is_non_jpg_format "$img_path"; then
        echo "执行步骤1：将${img_ext}格式转换为JPG"
        convert "$img_path" \
            -quality "$JPG_MAX_QUALITY" \
            -interlace Plane \
            -sampling-factor 4:2:0 \
            "$temp_jpg" || {
            echo "错误：格式转换失败，跳过该文件处理！"
            if [ -f "$temp_jpg" ]; then
                rm -f "$temp_jpg"
            fi
            return
        ...
        # 已是JPG格式，直接复制到临时文件
        cp -f "$img_path" "$temp_jpg" || {
            echo "错误：无法复制JPG文件到临时目录，跳过该文件处理！"
            return
        }
    fi
    ...
    if [ ! -f "$temp_jpg" ] || [ "$(get_image_size_kb "$temp_jpg")" -eq 0 ]; then
        echo "错误：临时文件生成失败，跳过该文件处理！"
        return
    fi

    # 第二步：分辨率调整 - 高度大于2160p时按比例压缩
    local current_temp_height=$(get_image_height "$temp_jpg")
    if [ "$current_temp_height" -gt "$MAX_HEIGHT" ]; then
        echo "执行步骤2：将高度从${current_temp_height}px压缩到${MAX_HEIGHT}px（保持比例）"
        convert "$temp_jpg" \
            -resize "x${MAX_HEIGHT}>" \
            -filter Lanczos \
            -quality "$JPG_MAX_QUALITY" \
            "$temp_jpg" || {
            echo "错误：分辨率压缩失败，跳过该文件处理！"
            if [ -f "$temp_jpg" ]; then
                rm -f "$temp_jpg"
            fi
            return
    ...
    # 第三步：体积压缩 - 大于5MB时压缩到5MB以下
    local current_temp_size=$(get_image_size_kb "$temp_jpg")
    if [ "$current_temp_size" -gt "$MAX_SIZE_KB" ]; then
        echo "执行步骤3：将体积从${current_temp_size}KB压缩到${MAX_SIZE_KB}KB以下"
        high_fidelity_compress_jpg "$temp_jpg" "$MAX_SIZE_KB"
    fi

    # 替换最终文件（覆盖原有 JPG 或 创建新 JPG，删除原格式文件）
...
main() {
    ...
    echo "处理流程：1.非JPG格式转JPG 2.高度>2160p压缩至2160p 3.体积>5MB压缩至5MB以下（均最大化保留细节）"
    echo "-------------------------"
